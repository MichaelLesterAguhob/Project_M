...
try
{
    ...
    // get max orderNo
    $query1 = "SELECT MAX(orderNo) FROM preparing_orders";
    $result1 = mysqli_query($con, $query1);
    $maxOrderNo = mysqli_fetch_array($result1);
    $max = $maxOrderNo[0];

    // get min orderNo
    $query2 = "SELECT MIN(orderNo) FROM preparing_orders";
    ...
    while($min <= $max)
    {
        $total = 0;
        // get min orderNo
        $query3 = "SELECT * FROM preparing_orders WHERE orderNo = '$min' ";
        $result3 = mysqli_query($con, $query3);

        $checkMinNo = "SELECT * FROM preparing_orders WHERE orderNo = '$min' ";
        ...
        if($cmnrRow > 0)
        {
            ...
            $data .='<div class="card" id="billReceipt">
                        <div class="card-header" style="background-color:skyblue;">
                                <h3 style="text-align:left; display:inline-flex;">Order # '.$min.'</h3> 
                                <p class="prepText">Preparing</p>
                                <button 
                                style="display:block;"
